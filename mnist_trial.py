...
class QuantumFunction(Function):
    ...
    def run(self, theta_vals: torch.tensor) -> torch.tensor:
        ...
        #If running on GPU, thetas is a torch.tensor that will live on GPU memory.
        #The observe function calls a .tolist() method on inputs which moves thetas from GPU to CPU.
...
def accuracy_score(y, y_hat):
    # Ensure tensors are on CPU and compute accuracy as a Python float
    y_cpu = y.detach().cpu()
    yhat_cpu = y_hat.detach().cpu()
    preds = (yhat_cpu >= classification_threshold).float()
    correct = (preds == y_cpu).sum().item()
    return correct / y_cpu.numel()
